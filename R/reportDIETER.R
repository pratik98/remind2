...
#' @export
reportDIETER <- function(dieterDatafile = "report_DIETER.gdx", output_dir=".") {
  ...
  out_annual <- annual_new <- dieter.tech.mapping <- dieter.tech.mapping.standalone <- NULL
  ...
  gdxToQuitteAnnual <- function(gdxfile){
    ...
    out_annual <-  out_annual %>% dplyr::mutate_(Unit =~if_else(grepl("\\(",Unit),Unit,"NA") )
  ...
  out_annual <- gdxToQuitteAnnual(dieterDatafile)
  ...
  dir.create(file.path(output_dir, sub_folder), showWarnings = FALSE)
  # save Dieter Data as a seperate mif & rds
  fwrite(annual_new, file.path(output_dir, sub_folder,"Dieter_Annual.mif"), append=F, sep=";", eol=EOL)
  read.quitte(file.path(output_dir, sub_folder,"Dieter_Annual.mif")) %>% 
    readr::write_rds(paste0(gsub(".mif","",file.path(output_dir, sub_folder,"Dieter_Annual.mif")),'.rds'), compress = 'xz') 
  ...
  # load main mif files
  remind_miffiles <- list.files(output_dir, pattern = "REMIND_generic", full.names = F)
  #miffile_name <- name_mif[!grepl("withoutPlu", name_mif)]
  for (mif_file in remind_miffiles) {
    
    name_mif <- file.path(output_dir, mif_file)
    stopifnot(typeof(name_mif) == "character")
    miffile <- fread(name_mif, sep=";", header=T)
    
  file.copy(from=name_mif, to=file.path(output_dir, sub_folder,mif_file), overwrite=TRUE, recursive=FALSE)
  fwrite(annual_remind, file.path(output_dir, sub_folder,mif_file), append=T, sep=";", eol=EOL)
  
  
  read.quitte(file.path(output_dir, sub_folder,mif_file)) %>% 
    readr::write_rds(paste0(gsub(".mif","",file.path(output_dir, sub_folder,mif_file)),'.rds'), compress = 'xz') 
  ...
  write.table(HourlyandAnnual, file.path(output_dir, sub_folder,"Dieter_Annualhourlyreport.csv"), sep = ";", row.names = F)
  
  # convert to  datatable
  # create mif and RDS
  data.table::setDT(HourlyandAnnual)
  EOL <- if (.Platform$OS.type == "windows") ";\r\n" else ";\n"
  fwrite(HourlyandAnnual, file.path(output_dir, sub_folder,"Dieter_Annualhourly.mif"), append = F, sep = ";", eol = EOL)
  
  
  read.quitte(file.path(output_dir, sub_folder,"Dieter_Annualhourly.mif")) %>% 
    readr::write_rds(file.path(output_dir, sub_folder, "Dieter_Annualhourly.rds"), compress = 'xz') 
}